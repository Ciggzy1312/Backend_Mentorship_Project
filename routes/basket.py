...
async def update_basket_by_id(basket: BasketBase, user: dict = Depends(get_current_user)):

    basketUpdated = await Basket.update_one({"userId": user["id"]}, {"$set": validator.remove_none_values(basket.dict())})
